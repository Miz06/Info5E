...
//$querySelectStatoPlichi = 'SELECT stato FROM db_FastRoute.plichi';
...
$querySelectPlichiGiorni = 'SELECT count(*) FROM db_FastRoute.consegnare WHERE data_consegna >= :data';
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['giorni'])) {
        $giorni = (int)$_POST['giorni']; // Assicurati che sia un numero intero
        $dataLimite = date('Y-m-d', strtotime("-$giorni days")); // Calcola la data limite

        try {
            $stm = $db->prepare($querySelectPlichiGiorni);
            $stm->bindValue(':data', $dataLimite); // Usa la data limite
            $stm->execute();

            $numeroConsegnati = $stm->fetchColumn();
            $stm->closeCursor();
        } catch (Exception $e) {
            logError($e);
        }
...
?>
    ...
    <div class="element">
        ...
        <?php if (isset($_POST['giorni']) && isset($numeroConsegnati)) { ?>
        <h4>Plichi consegnati negli ultimi <?php $_POST['giorni'] ?> giorni</h4>
        <hr>

        <?php echo 'Plichi consegnati: ' . $numeroConsegnati;}?>
    </div>
